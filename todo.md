...
- [ ] **P1 每账号代理**:账户配置加 `proxy` 字段(scheme://host:port,http/https/socks5,凭据内嵌)→ Web 层(SteamWebHandler/HttpClient 工厂)按账户注入 `HttpClientHandler.Proxy` → **SteamKit CM 连接代理调研**(连接工厂能力;不支持则如实报告 Web-only 折衷与自研 socks5 流选项)→ 代理凭据进脱敏黑名单 → 连通性自检 action + dashboard 账户编辑。
