...
- [ ] **P2 异常账号检测**:`check_account_standing` action(GetPlayerBans:VAC/community/trade/economy ban + limited 标记)→ 编排周期体检 + NotificationService 告警 → **编排联动自动隔离**(trade/economy ban 账号跳过 trade/market 任务、login 连败标记)→ dashboard 状态卡。
