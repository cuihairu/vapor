...
│   └── Performance/                      并发与压力
├── Vapor.ControlPlane.Tests/             (160 tests)
...
| Vapor.ControlPlane.Tests | 160 | REST API、SQLite job/审计存储、任务派发、账户编排、周期任务、通知、追踪 + WS 协议回放、报价同步查询 |
...
| **合计** | **1010** | (2026-09-12 基线;另 E2E 以真实子进程覆盖 Agent 主循环,单测统计测不到) |
...
| AccountApiTests | 18 | `/v1/accounts` REST(含 farm 状态、报价同步查询端点:fake agent 回报 200/502/202) |
