...
| SensitiveDataRedactorPropertyTests | 8 | 维护轮 FsCheck property:SanitizeLogValue 恰删控制字符三律(输出无控制字符/幂等/无控制字符输入恒等)、Redact 任意输入不抛、proxy URI 凭据段消失且占位符就位、JSON 敏感键任意大小写与分隔变体归 redacted、JSON 良性值往返、RedactValue 键敏感二分 |
...
| CompositionRootSmokeTests | 3 | 组装根冒烟(环境驱动真宿主 boot:存储 lambda/webhook sink/OTel/Swagger 开关 + `Vapor_API_RATE_LIMIT_PER_MINUTE` 限流器装配端到端) |
...
### 当前基线（2026-09-26，行覆盖 100.0% / 分支覆盖 100.0%）
...
> **Codecov 对照面（2026-09-23 维护轮对齐）**：codecov.yml project target 70%→100%（threshold 2→1pt，留给 Codecov 自行聚合同批 cobertura 与本地 coverage-summary.py max 合并的口径微差——真回退由上面 CI 硬门拦截，Codecov 的边际价值是第二道口径对照）；patch target 60%→100%（与 CI「新增行必须带测试」同语义）。**实测记录（GitHub statuses 原文）**：十三轮提交 c3546a6（最后一个带代码 diff 的提交）——`codecov/project: success, 100.00% (target 70.00%)`、`codecov/patch: success, 100.00% of diff hit (target 60.00%)`；维护轮 6789c6d（纯 docs，新 target 100/100 首验）——`codecov/project: success, 100.00% (target 100.00%)`、`codecov/patch: success, Coverage not affected`（docs diff 无代码行，patch 语义=无缺口）。聚合口径与本地合并一致，project/patch 双 100%，剩余缺口 0。
...
> 2026-09-22 分支缺口冲刺第七轮（收口轮续,逐轮棘轮:+82 测试全绿,分支 6022/6165=97.7% → **6131/6165=99.4%**,门禁 97.68 → **99.44**）。Agent 99.5% → **100%（194/194 收满,第二个分支全绿业务程序集）**:执行器 snake_case code 别名臂、MaFileImport CLI 缺 SteamId 以 account_name 落库臂、StartExecuteSpan 带 listener 的 Activity task 标签臂（const SourceName 模式同入 TaskScheduler 两 listener 测试,规避 VaporTracing cctor 重入——listener.ShouldListenTo 里访问 `VaporTracing.Source.Name` 字段会触发 cctor,cctor 内 new ActivitySource 反向回调 listener 取到未初始化 null 炸 TypeInitializationException）。ControlPlane 96.8% → 99.4%（2060/2072）:**本轮最大坑（顶层语句局部函数反射）**:WriteAuditLog/GetAuditActor 以生成名 `<<Main>$>g__…|0_N` 发出（裸名 GetMethod 返 null,strings 验 DLL 确认）;`typeof(Program)` 解析到 namespace 下空 `public partial class Program;`,顶层语句类须 `typeof(Program).Assembly.GetType("Program", throwOnError:false)` 获取,再 `GetMethods(NonPublic|Static).First(m => m.Name.Contains(…) && 参数数匹配)`。X-Forwarded-For 语义修正:空串被 IsNullOrEmpty 拦落 RemoteIpAddress,空白串被原样采用为 actor（IsNullOrEmpty 对空白返回 false）,两用例分开落。AchievementsUnlock 主任务 Failed 无 Error 的 502 状态回退臂。Reconciler 残留 7 条全清:747 stop 派发臂（Idling=false+BoostPlayingApps 非空）;1145/1158 is_our_offer 实测行号漂移,真缺口是缺键 TryGetProperty-F 与「字符串 "true" 经 is true 类型检查读作非我方→gift 可接受」null 臂;1767 真身是 standing_alert payload economyBan/vacBanned TryGetValue 键缺失双 null 臂;1898 AcceptingOffer 丢失偏差臂（TradeOffersToAccept 须一并清,否则同 pass 二次派发清 LastDeviation）;2216 FarmQueueCheckedAt→QueueRefreshedAt 逐字回传。Program 3192/3202 **证伪「结构性不可达」**:单收集证明原测试与基线逐位重复（TestServer 与 DefaultHttpContext 的 RemoteIpAddress 均 null）,真缺口是「带非 null RemoteIpAddress 直调 WriteAuditLog」组合,补直调即 100%。Plugins.Core 99.0% → 99.6%:PluginLoadContext deps.json runtimeTargets native 段（rid/assetType:native 定位 .so,运行时真库拷改名后反射 LoadUnmanagedDll 验句柄非零,Linux 守卫+ALC day-one best-effort 清理）。MobileAuthenticator 98.4% → 99.4%:GetTradeConfirmations/ConfirmAll 的 confirmations-null 零计数双臂与 listing 失败 error-null 回退臂。Steam.Core 97.9% → 99.5%（2999/3015）:Actions 族——四个 TryParse/CLR-string 守卫 skip 臂（喂 "abc"/"1.5"）;FindDuplicates:108 分页三条件六臂与 :232 keep 双失败臂;GetInventory:233/Swap:196 error-null 回退消息臂;Swap:203 TradeOfferId-null 臂;Swap:253 DescribeAsset `?? ??` 回退链;GetPlaytime:124/DataActions:438 cache 返 null `?? []` 臂;DataActions:535 FinalFormatted/Final 双 null 组合;UnlockAchievements:108 reset 全败三元;杂项:CardSwapMatcher:123 Amount>0 臂、TradeRateLimiter:252 `_release?.Invoke()` null 臂（**勘误:第六轮声称已收,实测从未构造 lease,本轮 new TradeRateLease(null!) 真收**）、VaporCrypto:170 `file:` 前缀臂、MaFileParser:130 双 F 臂、SessionManager:372 accessToken-null 短路、FileCredentialStore:137 无 expiry 键臂（与 FindDuplicates:232 一并证伪「疑陈旧基线」——都是真实缺口）。**补收漏派矿点 4 臂**:CredentialStoreRotator:89/102（wrong-key+NullLogger 一次调用同收逐账户失败与 Rotation aborted 两条 LogError 非 null 臂）与 :123（成功+NullLogger 摘要臂）、CheckAccountStandingAction:61 webHandler-null 短路（FetchOverride 绕过早守卫后 payload 缺 steam_id 落 "SteamID unknown";现有三测试恰缺 handler-null 臂）。定性入册 34 条无剩余可测矿点:真网探测（CheckProxyAction:63 ProbeAsync 打 api.ipify.org）;恒非 null（PluginLoader:187 CreateInstance、MobileConfirmationClient:237/SteamAccountStandingClient:94 ValueKind==String 保证 GetString、Program:2993 token.Count 守卫、AccountStore:60/90+ConfigStore:70 Build 恒造 Version、AuthenticatorActions:186 LastSyncedAt、NotificationSinks:191 lastError catch 内赋值后 throw、RedisVaporCache:235/254 真 Redis 枚举）;**lifted nullable 死臂（第七轮新认知）**:CardSwapMatcher:141/LootAction:224 的 `TradabilityDate is null || TradabilityDate <= now` 恒 3/4——右侧 nullable 比较被 coverlet 拆 HasValue-T/F 两结果位,左侧 is null 为 F 时恒有值,HasValue-F 臂结构性不可达;死臂（Reconciler:137 WaitForNextTickAsync false 臂取消经 OCE 解卷/1178 装箱 true ToString="True" 必被左侧先中/1633 ActiveJobId 与 Action 恒成对写入/2181 同方法早处取样时钟单调恒正、SensitiveDataRedactor:172 零宽组恒 Success、RedactingLoggerProvider:36 双位同源绑定 state 恒非 null、MonitoringPlugin:200 唯一调用点在 _logger 无条件赋值后）;竞态窗口不可注入（SessionManager:230/244 基线误标 TryAdd 行,真身 L210 已被 Barrier 测试收、SqliteCrawlStore:261 claim 乐观锁需真并发抢行）;恒 long（SqliteAuditStore:121/SqliteCrawlStore:352 COUNT(*) is long false 臂）;SteamKit2 反射壳（SteamClientManager:40/48）。验证:全量串行绿（10 段全 attempt 1,2860 测试）,format 门禁过,CI 终态见提交后监控。

> 2026-09-23 分支缺口冲刺第八轮（复核轮,逐轮棘轮:+12 测试全绿,分支 6131/6165=99.4% → **6144/6165=99.7%**,门禁 99.44 → **99.65**）。**方法论:全量复核第七轮 34 条定性,翻案 11 条（+13 位）**——「恒非 null/唯一调用点」定性只约束自然路径,`?.`/`??`/is-null 守卫等防御臂存在的语义就是接住不自然形态,**反射注入不自然形态正是其测试法**:AccountStore:60/90+ConfigStore:70 反射塞 null-Version 记录入私有字典（existing.Version? 防御臂直接驱动,推翻「Build 恒造 Version」）;MonitoringPlugin:200 未初始化实例反射直调 StartSessionPump 命中 `_logger?.` null 臂（推翻「唯一调用点在赋值后」）。ControlPlane +6:Reconciler:1633 反射造缺 action 半边的 runtime + Invoke SettleActiveJobAsync 走登录兜底、:2181 FarmStatsStartedAt 置未来时间令 hours<=0;**SqliteCrawlStore:261 竞态臂的确定性替身——反射取 `_connection` 建 `RAISE(IGNORE)` BEFORE UPDATE trigger 令 claim UPDATE 静默归零行数,恰为乐观锁被并发抢走的确定性等价**;MetricsHttpServer:97 warmup park 后换 NullLogger 实例走 accept 错误 LogDebug 非 null 臂。Steam.Core +5:CheckProxyAction:63 不设 ProbeOverride+预取消 token 收 `?? ` 失败结果臂（**修正第七轮机制预判:预取消 OCE 被 ProbeAsync 内部 catch(Exception) 吞掉,探测以失败结果返回而非外层 when 重抛**）;RedactingLoggerProvider:36 formatter 经 inner.Log 逃逸给任意 provider,sink 端按 `Func<object?,Exception?,string>` 契约传 null state 即该防御臂的存在理由（推翻「state 恒非 null」）;SessionManager:348 SessionEvent.NewState 是可空属性,省略即合法生产者形状——反射注入 `_eventChannel` 写入无 NewState 事件,泵转发空串（推翻「全事件 NewState 恒非 null」）。**定性修正 2 条**:Program:2993 原定性行号错位——真身为 `token[0] ?? string.Empty` 右臂被同条件 `!IsNullOrWhiteSpace(token[0])` 逻辑锁死（补测试实测仍 1/2 确证死臂）;MetricsHttpServer:122 实为 `string.Split` 恒非空数组的恒真死臂。**本轮最大发现（行覆盖轮盘事件）**:首轮串行 15881/15882——SessionManager:126（TryAdd 竞态败者 else）hits=0,第三轮的「Barrier 16 线程确定性收口」实为**高概率**而非确定:同步方法体无 await 可挂起,首个被调度线程可一路跑完构造+TryAdd,其余线程 TryGetValue 全部命中 incumbent 走 existing 臂（本轮 15 线程全走 existing,75 次应然输家 0 命中;CI 行门禁 100% 下即轮盘）。处置:**三元化重构**——`TryAdd ? await CompleteCreateAsync(...) : HandleDuplicateCreateRace(...)` 同行条件表达式,成功路径供行 hits、竞态臂序列点出分母;coverlet 对该形态不生成 branch 探测点,107 行 TryAdd-false 的 1/2 竞态分支缺口一并消失（分支净 +1）。教训入册:「概率高」≠「确定」——100% 行门禁之下概率命中行必须结构性消除,成功臂与不可测臂共享序列点行是通用解（else 折叠只解决孤立花括号行,不解决 else 体语句行）。维持定性 21 条（SessionManager:230/244 因插入 CompleteCreateAsync 漂移至 233/247）:lifted nullable HasValue-F（CardSwapMatcher:141/LootAction:224 恒 3/4）、RESP 协议恒 bulk string（RedisVaporCache:235/254）、零宽正则组恒 Success（SensitiveDataRedactor:172）、竞态窗口无注入点（SessionManager:233）、生产侧从不 Complete（SessionManager:247）、SteamKit2 版本形变防御（SteamClientManager.cs:40/48 SteamAuthTokenProvider 类）、Count<Max 守卫穷举排除（TradeRateLimiter:199）、for(;;) 必进一次（NotificationSinks:191）、装箱 bool ToString 恒 "True"（Reconciler:1178）、timer 局部 using 不可触（Reconciler:137）、COUNT(*) 恒装箱 long（SqliteAuditStore:121/SqliteCrawlStore:352）、ValueKind==String 契约（MobileConfirmationClient:237/SteamAccountStandingClient:94 21/22）、逻辑锁死死臂（Program:2993/MetricsHttpServer:122）、Split 恒真（MetricsHttpServer:122）、成功路径必经唯一赋值点（AuthenticatorActions:186）、CreateInstance 恒非 null（PluginLoader:187）。验证:全量串行绿（10 段全 attempt 1,2872 测试）,format 门禁过,CI 终态见提交后监控。

> 2026-09-23 分支缺口冲刺第九轮（复核深化轮,逐轮棘轮:+4 测试全绿,分支 6144/6165=99.7% → **6148/6165=99.8%**,门禁 99.65 → **99.72**）。**方法论:对第八轮维持的 21 条定性再逐条精读源码复核,翻案 4 条（+4 位）**——「定性只约束当时的判定深度」,轻量判定与逐行精读之间仍有矿。①SessionManager:247（SubscribeAllEvents）:1/2 缺的是**枚举正常完成位**——第六轮只写了取消臂（WhenCallerCancels 断言 OCE）,而 channel Complete 是另一合法循环出口:反射 `Writer.Complete()` 后 MoveNextAsync 正常返回 false 不抛不挂（第六轮泵测试 ExitsThroughChannelCompletion 同款手法,生产侧「从不 Complete」是事实描述不是逻辑必然）。②TradeRateLimiter:199:私有 PeekEarliest 的空窗口防御臂——「Count<Max 守卫穷举排除」只约束自然路径,反射 Invoke 直驱空 Queue 命中 `Count > 0` F 位（固定 `_utcNow` 断言回退时钟,不依赖真实时间）。③RedisVaporCache:235/254:**推翻第八轮「mock 造 null 属捏造协议违规」的定性**——FakeRedis 本就是 Moq mock 架构,防御臂（`(string?)key ?? string.Empty`）的语义就是接不自然形态,与反射塞 null-Version 同构:mock `Keys` 返回 `default(RedisKey)`、`SetMembers` 返回含 `RedisValue.Null` 的数组,合并臂命中且索引批量更新不抛（断言走 SetRemovals 的纯 string 通道,绕开 RedisKey.ToString 对空键的 null 语义）。**维持 17 条（理由精确化）**:Reconciler:1178 `||` 右操作数 T 位不可达是逻辑蕴含——`value is true` ⇒ `ToString()="True"` ⇒ OrdinalIgnoreCase Equals=T ⇒ 左臂恒短路,喂 "false" 只补左 F 右 F 组合;SensitiveDataRedactor:172 `(?<suffix>"?)` 组在主模式序列中恒参与匹配尝试（零宽也算参与）→ Success 恒 true;NotificationSinks:191 循环内成功必 return、失败必设 lastError,到 throw 行 lastError 逻辑必然非空;AuthenticatorActions:186 LastSyncedAt 非虚密封属性且 SyncAsync 成功必经唯一赋值点;SteamAuthTokenProvider:40/48 typeof 反射查找在版本锁定下恒命中;其余维持:lifted nullable HasValue-F（CardSwapMatcher:141/LootAction:224）、竞态嵌套窗口（SessionManager:233）、ValueKind==String 契约（SteamAccountStandingClient:94 21/22、MobileConfirmationClient:237）、逻辑锁死/恒真（Program:2993、MetricsHttpServer:122 Split 恒非空）、CreateInstance 恒非 null（PluginLoader:187）、COUNT(*) 恒装箱 long（SqliteAuditStore:121/SqliteCrawlStore:352）、timer 局部 using 不可触（Reconciler:137）。验证:全量串行绿（10 段全 attempt 1,2876 测试）,format 门禁过,CI 终态见提交后监控。

> 2026-09-23 分支缺口冲刺第十轮（复核收口轮,逐轮棘轮:+0 测试纯源码等价改写,分支 6148/6165=99.8% → **6141/6153=99.8%**,门禁 99.72 → **99.80**）。**方法论:对第九轮维持的 17 条定性逐行精读复核,全部维持**（每条均有逻辑必然性论证,无翻案空间）;实质产出为 **5 处语义严格等价改写消除死臂**——第九轮定性出「逻辑锁死」的臂,与其留着 coverlet 为之铸造的不可达探测位,不如把死臂从代码里语义等价地移走（先例:fd1ae2e 删死代码、第八轮三元化）:①CardSwapMatcher:141/LootAction:224 lifted nullable 死位——`TradabilityDate is null || TradabilityDate <= now` 的 lifted 比较被 coverlet 拆出 HasValue-F 探测位,而 `{ get; init; }` 不可变属性两次求值恒同值 → 死位;改写为 `is null || TradabilityDate.Value <= now`,非 lifted 比较不生成该探测位（.Value 安全性由 is-null 臂先行排除）。②MetricsHttpServer:122 `parts.Length > 0 ? parts[0] : string.Empty`——string.Split 恒至少一个元素,守卫恒真,改 `parts[0]`（下一行 parts.Length>1 守卫是真防御,保留）。③PluginLoader:187 `Activator.CreateInstance(pluginType) ?? throw`——CreateInstance(Type) 仅 type-less COM 场景返 null,普通 .NET 类型要么构造成功要么抛（MissingMethodException catch 保留）,改 `CreateInstance(pluginType)!`。④Reconciler:1178 `Equals(...) || value is true`——第九轮已论证右 T 位是逻辑蕴含死位（`value is true` ⇒ `ToString()="True"` ⇒ 左臂 OrdinalIgnoreCase Equals 恒短路）,本轮把论证落实为代码:删冗余右臂。**分母口径变化披露**:分支分母 6165→6153（coverlet 探测位 −12:CardSwap/Loot/PluginLoader/MetricsHttp 各 −2、Reconciler −4）,源码条件表达式一个未动;分子 6148→6141——删掉的 12 位中 7 位原已覆盖、5 位原为缺口位（缺口 17→12）;行分母 15884→15883（PluginLoader 两行并一行）,行覆盖保持 100.0%（15883/15883）。Monitoring 与 Plugins.Core 两程序集分支升至 100.0%。比例上升不违「覆盖率不回退」铁律——删除的是不可达探测位,可测集合本身变纯。验证:Debug 全量 10 项目 2876 测试全绿、format verify 过、显式 Release build 过、全量串行绿（10 段全 attempt 1,0 重试）,CI 终态见提交后监控。

> 2026-09-23 分支缺口冲刺第十一轮（复核收口轮二,逐轮棘轮:+0 测试纯源码等价改写,分支 6141/6153=99.8% → **6133/6137=99.9%**,门禁 99.80 → **99.93**）。**方法论:对剩余 12 条缺口逐行精读判定,8 处等价改写消除死臂、4 条维持**。8 处改写（各消 1 缺口位 + 1 已覆盖位,分母 −16）:①SensitiveDataRedactor:172 `Groups["suffix"].Success ? Value : ""` → `Value`——suffix 组在主模式路径（可选零宽）恒参与 ⇒ Success 恒 true,且 .NET 契约未匹配组 Value 恒 Empty,守卫恒真;②SteamAccountStandingClient:94 `GetString() ?? "unknown"` → `GetString()!`——STJ 契约 String token ⇒ 恒非 null（第九轮已论证,本轮落实代码）;③MobileConfirmationClient:237 `GetString()?.Trim()...` → `GetString()!.Trim()...`——同上;④Program:2993 `(token[0] ?? string.Empty)` → `token[0]!`——外层 `IsNullOrWhiteSpace(token[0])` 守卫已排除 null;⑤NotificationSinks:191 `throw lastError ?? new HttpRequestException(...)` → `throw lastError!`——循环不变式:成功即 return、失败两赋值点（响应失败/异常 catch）全覆盖、OCE rethrow 不经 throw 行,lastError 逻辑必然非空;⑥AuthenticatorActions:186 `LastSyncedAt?.ToString("O")` → `LastSyncedAt!.Value.ToString("O")`——186 可达 ⇒ SyncAsync 正常返回 ⇒ 唯一赋值点已执行;⑦⑧SqliteAuditStore:121/SqliteCrawlStore:352 `result is long count ? (int)count : 0` → `(int)(long)result!`——两处 CommandText 恒 `SELECT COUNT(*)`（BuildQueryCommand 尾部唯一 SQL）,SQLite COUNT 聚合恒返回装箱 long（0 行 ⇒ 0 非 NULL）,引擎契约与 PluginLoader 的 CreateInstance 同级。**4 条维持（含新论证）**:Reconciler:137——WaitForNextTickAsync F 位 = timer Dispose 后返 false,局部 `using` timer 无外界引用,ExecuteAsync 存续期间无人能 Dispose,结构性死位且 while 条件无改写空间;SessionManager:233——TryGetValue F 位需键在 [TryAdd 失败, TryGetValue] 的**无 await 同步窗口**（纳秒级）内被第三方移除,比第八轮 126 行竞态连「高概率」都不可得;SteamClientManager:40/48——GetConstructor/GetMethod 的 `?? throw` 依赖 SteamKit2 **包结构**而非 .NET 运行时契约,包升级真可能破坏,版本形变防御臂有真实 fail-fast 价值（清晰消息 vs NRE）,与 PluginLoader:187 的运行时契约本质不同,故不改写。**分母口径披露**:分支分母 6153→6137（探测位 −16,源码条件表达式未增减）、分子 6141→6133（每处删 1 缺口位 + 1 已覆盖位）,缺口 12→4;行分母 15883→15892（改写引起序列点 +9,全部命中,行覆盖保持 100.0%）。ControlPlane 与 MobileAuthenticator 分支升至 100.0%,仅剩 Steam.Core 3 条 + ControlPlane 1 条。教训:「防御性写法」要区分防御对象——运行时契约（恒真）与外部包契约（可破坏）的 ?? throw 语义价值完全不同,前者可等价消除,后者必须保留。验证:Debug 全量 10 项目 2876 测试全绿、format verify 过、显式 Release build 过、全量串行绿（10 段全 attempt 1,0 重试）,CI 终态见提交后监控。**第十一轮收尾修复（行门禁伪行轮盘）**:首次 CI coverage job 行门禁红——CI 行 15890/15892 差 2 行（Steam.Core）而本地全命中,`gh run rerun` 转绿（15892/15892）证实非确定性轮盘。根因:8 处改写的多行独立注释被 coverlet 把跨行语句序列点挂到注释行上,铸造 9 个伪行分母（MA +4、Steam.Core +5、ControlPlane 0——伪行取决于语句形态,跨行表达式前的注释高发）,其中 2 个 CI 首跑未命中。结构性修复:8 处多行注释全改行尾注释（不新增物理行 ⇒ 序列点必挂代码行 ⇒ 伪行不可能存在）,论证全文保留在行尾与本册;行分母 15892→15883（MA 595→591、Steam.Core 6122→6117）,行覆盖保持 100.0%,分支 6133/6137 不变。教训:覆盖率敏感代码的注释形态是覆盖分母的一部分——独立多行注释插在多行语句之前会进分母,行尾注释不会;守则「改覆盖率敏感代码用行尾注释」。修复后复验:Debug build 0 警告、format verify 过、显式 Release build 0 警告、全量串行绿（10 段全 attempt 1,0 重试）、门禁预验 GATE_CHECK_EXIT=0。
