...
| MobileAuthenticator | 100.0% | 99.4% (318/320) |
...
| Protocol | 100.0% | （无分支行） |
| ControlPlane | 100.0% | 99.8% (2063/2068) |
| Steam.Core | 100.0%（取消/竞态臂经确定性测试与排除定性收尾，见下） | 99.8% (3006/3011) |
...
| **合计** | **100.0%** (15883/15883) | **99.8%** (6141/6153) |

分支覆盖门禁：CI `--min-branch 99.80`（基线 6141/6153 的未舍入值为 99.8050%，门禁取 99.80——当前过、丢一个条件 99.7887% 即红；2026-09-23 分支缺口冲刺第十轮后设点，百分比一律工具计算）。行覆盖 100% 不蕴含分支覆盖 100%：一行执行过不等于它的每个布尔子条件结果都被取到。
