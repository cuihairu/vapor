...
分支覆盖门禁：CI `--min-branch 96.83`（基线 5967/6165 的未舍入值为 96.837%，门禁取 96.83——当前过、丢一个条件 96.821% 即红；2026-09-22 分支缺口冲刺第三轮后设点）。行覆盖 100% 不蕴含分支覆盖 100%：一行执行过不等于它的每个布尔子条件结果都被取到。
