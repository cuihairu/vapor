...
| Protocol | 100.0% | （无分支行） |
| ControlPlane | 100.0% | 100.0% (2059/2060) |
...
| **合计** | **100.0%** (15885/15885) | **100.0%** (6134/6135) |

分支覆盖门禁：CI `--min-branch 99.98`（基线 6134/6135 的未舍入值为 99.9837%，门禁取 99.98——当前过、丢最后一个条件（Reconciler:137，已定性结构性死位）99.9674% 即红；2026-09-23 分支缺口冲刺第十二轮后设点，百分比一律工具计算）。行覆盖 100% 不蕴含分支覆盖 100%：一行执行过不等于它的每个布尔子条件结果都被取到。
