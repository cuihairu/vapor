...
| Protocol | 100.0% | （无分支行） |
| ControlPlane | 100.0% | 99.4% (2060/2072) |
| Steam.Core | 100.0%（取消/竞态臂经确定性测试与排除定性收尾，见下） | 99.5% (2999/3015) |
...
| **合计** | **100.0%** (15882/15882) | **99.4%** (6131/6165) |

分支覆盖门禁：CI `--min-branch 99.44`（基线 6131/6165 的未舍入值为 99.4485%，门禁取 99.44——当前过、丢一个条件 99.4323% 即红；2026-09-22 分支缺口冲刺第七轮后设点。设点时一度心算成 96.837 抬到 96.83，CI 门禁红、设点者被自家门禁拦下，以此条勘误留档；此后百分比一律工具计算）。行覆盖 100% 不蕴含分支覆盖 100%：一行执行过不等于它的每个布尔子条件结果都被取到。
